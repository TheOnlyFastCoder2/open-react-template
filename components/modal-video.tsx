...
import { animFlip } from '@/app/animations';
import { useSignal } from '@/utils/stm/react/react';
  ...
  return (
    <div className="relative">
      ...
      >
        ...
        {/* Play icon */}
        <span className="absolute cursor-pointer p-2.5 before:absolute before:inset-0 before:rounded-full before:bg-gray-950 before:duration-300 group-hover:before:scale-110">
          <span
            className="relative flex items-center gap-3"
            onMouseEnter={() => (isEnter.v = true)}
            onMouseLeave={() => (isEnter.v = false)}
            onTouchStart={() => (isEnter.v = true)}
            onTouchEnd={() => (isEnter.v = false)}
          >
            <Spring
              isActive={isEnter}
              classInner="rounded-[50%]"
              spring={{
                scale: {
                  values: { default: 1, active: 1.2 },
                  stiffness: 80,
                  damping: 4,
                },
                rotate: {
                  values: { default: 1, active: 360 },
                  stiffness: 80,
                  damping: 10,
                },
                boxShadow: {
                ...
                shadowColor: {
                  values: {
                    default: [58, 65, 109, 0.25],
                    active: [58, 65, 109, 0.45],
                  },
                  stiffness: 120,
                  damping: 10,
                },
